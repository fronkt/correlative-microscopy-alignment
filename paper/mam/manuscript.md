...
*Alt text:* A two-panel schematic. Panel A is a flow diagram of the verified coarse-to-fine wrapper, showing a direct match producing an incumbent transform, conditional tile-search and zoom stages, and a mutual-information verifier gate that admits a candidate only if it scores higher than the incumbent. Panel B shows a wide-field micrograph with successively smaller crop boxes drawn on it, illustrating the field-of-view ladder in which appearance is fixed and only scale changes.
